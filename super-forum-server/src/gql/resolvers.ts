...
// 类型标注和类型断言的区别
// 两者都是为变量指定类型，但是标注是ts无法推断类型unknown, 就不能对变量进行操作，所以需要通过标注指定类型
// 类型断言则是，ts 可以推断出类型，但是无法确定是哪个，才符合要求，所以需要断言一下，告诉ts, 就这个类型，别猜了
const resolvers: IResolvers = {
	UserResult: {
		// __resolveType 函数通常与联合类型（Union Types）和接口（Interfaces）一起使用。它的作用是在运行时确定一个对象的具体类型。
		// graphql 碰到一个联合类型的时候，需要这个 __resolveType 对类型进行具体判断，确定类型
	...
	Mutation: {
		...
		// 写代码本质是把代码写出来，而不是像做题一样把代码默写出来；
		// 就拿 GraphQL这个框架来说，不是要你把他的各种写法背下来，而是根据项目所选的技术栈，利用各种文档，把项目框架搭建起来，然后写通一个接口，其他接口就照着写即可。
		// 不要老是把以前做题的习惯，带到做项目，写代码中来，在做项目的过程中，不会问你具体的 API, 因为 技术栈不同，版本不同，API 都会变化。所以说，除了技术栈的基础使用，更多地要在意解决方案的设计与实现
		// 这个学习方式也可以用到其他编程语言的领域。
